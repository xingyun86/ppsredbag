 ...
 */
public class WXAccessbilityJob extends BaseAccessbilityTaskJob {

    private static final String TAG = "WXAccessbilityJob";

    /** 微信的包名*/
    private static final String WECHAT_PACKAGENAME = "com.tencent.mm";

    /** 红包消息的关键字*/
    private static final String WX_REDBAG_TEXT_KEY = "[微信红包]";

    /** 不能再使用文字匹配的最小版本号 */
    private static final int USE_ID_MIN_VERSION = 700;// 6.3.8 对应code为680,6.3.9对应code为700，6.5.10对应code为1080
        ...
        if(eventType == AccessibilityEvent.TYPE_NOTIFICATION_STATE_CHANGED) {
            ...
            if(!texts.isEmpty()) {
                for(CharSequence t : texts) {
                    ...
                    if(text.contains(WX_REDBAG_TEXT_KEY)) {
                        openNotify(event);
                        break;
                    }
                }
    ...
    private void openRedbag(AccessibilityEvent event) {
        ...
        else if(event.getClassName().toString().contains("com.tencent.mm.plugin.luckymoney.ui.En_")) {
            //适配6.5.10
            //点中了红包，下一步就是去拆红包
            handleLuckyMoneyReceive();
        }
    ...
    private void handleLuckyMoneyReceive() {
        ...
        int event = getConfig().getWechatAfterOpenRedbagEvent();
        if(event == Config.WX_AFTER_OPEN_REDBAG) { //拆红包
            if (getWechatVersion() < USE_ID_MIN_VERSION) {
                ...
                if(list == null || list.isEmpty()) {
                    ...
                    if(l == null || l.isEmpty())
                    {
                        l = nodeInfo.findAccessibilityNodeInfosByText("发了一个红包，金额随机");
                    }
                    if(l != null && !l.isEmpty()) {
        ...
        } else if(event == Config.WX_AFTER_OPEN_DETAIL) { //看一看
    ...
    private void handleChatListRedbag() {
        AccessibilityNodeInfo nodeInfo = getService().getRootInActiveWindow();
        if(nodeInfo == null) {
            Log.w(TAG, "rootWindow为空");
            return;
